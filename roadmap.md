...
- [x] Deploy 2026-09-19: `python -m modal deploy api.py` OK — `run_editorial_layout_job`
      created, `editorial_layout_adapter: bfl_editorial_layout` registered. First deploy
      showed `modules.editorial_layout: false` because the Mac had the stale pre-Module-C
      `advanced.py`; after swapping in the current file and redeploying, health reads
      `modules: {outpaint: true, product_scene: true, editorial_layout: true}`.
- [x] Registry migration `editorial_layout:v1` applied (staging/research-only row).
- [x] Config hash set 2026-09-19 via the managed Supabase connection (Python
      canonicalisation, verified identical to `registry.compute_config_hash`):
      `ebb4603df70ac6a6b5fb0ebd357a3da3837d917f7362d79082cf43cc3ca4c0c6`. Read back and
      confirmed; row stays `testing` / `research_only` / `internal`, prod + Studio off.
